...
function scCalculate() {
  ...
  const lines = [
    ...
    'rho_hot (' + scCondTemp + 'C) = ' + engRound(rhoHot, 4) + ' Ohm*mm2/m  [rho(T)=rho20*(1+alpha*(T-20))  IEC 60228 Annex B]',
    ...
    '=== Source impedance (complex) ===',
    isSinglePhase
      ? '|Zs| = U0 / Ik_sup = ' + engRound(U0, 4) + ' / ' + Ik_sup_A.toFixed(0) + ' = ' + fmtmOhm(Zs_mag) + '  [IEC 60909-0 §4.2]'
      : '|Zs| = U_LL / (sqrt(3) * Ik_sup) = ' + engRound(U_LL,4) + ' / (1.732 * ' + Ik_sup_A.toFixed(0) + ') = ' + fmtmOhm(Zs_mag) + '  [IEC 60909-0 §4.2]',
    'X/R ratio = ' + XR,
    ...
    '',
    '=== Cable impedances ===',
    'Rf_max (20C) = rho_ref * L / S   = ' + fmtmOhm(Rf_max) + '  [IEC 60909-0 §3.4, used for max Ik]',
    'Rf_hot (' + scCondTemp + 'C) = rho_hot * L / S = ' + fmtmOhm(Rf_hot) + '  [IEC 60909-0 §3.4, used for min Ik]',
    ...
    ...(isSinglePhase ? [
      ...
      'Ik3_max = c_max * U0 / Z3_max = ' + C_MAX + ' * ' + engRound(U0,4) + ' / ' + fmtmOhm(Z3_max) + ' = ' + fmtIkVal(Ik3_max) + '  [IEC 60909-0 §4.7.1]',
      'Ik3_min = c_min * U0 / Z3_min = ' + C_MIN + ' * ' + engRound(U0,4) + ' / ' + fmtmOhm(Z3_min) + ' = ' + fmtIkVal(Ik3_min) + '  [IEC 60909-0 §4.7.1]',
      '',
      'Ik2_max = c_max * U_LL / (2 * Z2_max) = ' + C_MAX + ' * ' + engRound(U_LL,4) + ' / (2 * ' + fmtmOhm(Z2_max) + ') = ' + fmtIkVal(Ik2_max) + '  [IEC 60909-0 §4.8]',
      'Ik2_min = c_min * U_LL / (2 * Z2_min) = ' + fmtIkVal(Ik2_min) + '  [IEC 60909-0 §4.8]',
      '  [Derivation: I"k2 = c*U_LL/(Z(1)+Z(2)) = c*U_LL/(2*Z3), yields exactly (sqrt(3)/2)*Ik3 per IEC 60909 §4.3]',
      '',
    ]),
    'Ik1_max = c_max * U0 / Z1_max = ' + C_MAX + ' * ' + engRound(U0,4) + ' / ' + fmtmOhm(Z1_max) + ' = ' + fmtIkVal(Ik1_max) + '  [IEC 60909-0 §4.9]',
    'Ik1_min = c_min * U0 / Z1_min = ' + C_MIN + ' * ' + engRound(U0,4) + ' / ' + fmtmOhm(Z1_min) + ' = ' + fmtIkVal(Ik1_min) + '  [IEC 60909-0 §4.9]',
    '',
    '=== Trip verification ===',
